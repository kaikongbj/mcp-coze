...
async fn test_real_api_integration() {
    ...
    // 直接设置API密钥和参数
    let api_token = "pat_8NU7DXjMPg4O7rg4tbt8ZYzKkHRIMTZ8SKANbdYdjf0vMPKR7CbKsn0biE9TKcDi".to_string();
    ...
    // 测试获取知识库列表
    match client.list_datasets(&space_id, None, None, Some(1), Some(50)).await {
        Ok(response) => {
            println!("✅ 真实API测试成功！");
            ...
            if !response.datasets.is_empty() {
                ...
                for (i, dataset) in response.datasets.iter().enumerate() {
                    ...
                    let actual_file_count = dataset.file_list.as_ref().map(|files| files.len()).unwrap_or(0);
                    ...
                    if api_reported_count == actual_file_count {
                        ...
                        println!("      ⚠️  数量不匹配: API={}, 实际={}", api_reported_count, actual_file_count);
                    }
                ...
                // 计算准确性
                let accurate_count = response.datasets.iter()
                    .filter(|d| d.document_count == d.file_list.as_ref().map(|f| f.len()).unwrap_or(0))
                    .count();
                
                println!("   准确报告的数据集: {} / {}", accurate_count, response.datasets.len());
                println!("   准确率: {:.1}%", (accurate_count as f64 / response.datasets.len() as f64) * 100.0);
...
async fn test_api_endpoints() {
    ...
    let client = match CozeApiClient::new(base_url, api_token) { Ok(c) => c, Err(e) => { println!("跳过: 客户端创建失败 {}", e); return; } };
    let space_id = env::var("COZE_SPACE_ID").unwrap_or_else(|_| "default_space".to_string());
    
    // 测试各种端点
    let test_cases = vec![
        ("list_datasets", || async {
            client.list_datasets(&space_id, None, None, Some(1), Some(5)).await
        }),
    ];
...
async fn test_pagination() {
    ...
    let client = match CozeApiClient::new(base_url, api_token) { Ok(c) => c, Err(_) => { return; } };
    let space_id = env::var("COZE_SPACE_ID").unwrap_or_else(|_| "default_space".to_string());
    ...
    let result = client.list_datasets(&space_id, None, None, Some(1), Some(page_size)).await;
...
async fn test_filtering() {
    ...
    let result = client.list_datasets(&space_id, Some(test_name.as_str()), None, Some(1), Some(10)).await;
