...
async fn debug_chat_request_format() {
    ...
    let payload = serde_json::to_value(&request).map_err(|e| format!("Serialization error: {}", e)).unwrap();
    println!("\n修复后的API请求格式:");
    ...
    assert!(json_str.contains("\"additional_messages\""), "additional_messages字段缺失");
}
